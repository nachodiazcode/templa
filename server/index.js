 ...
 * Supabase (public.orders) es la fuente de verdad cuando hay credenciales.
 * El JSON local se mantiene como espejo/respaldo para desarrollo sin DB.
 */
...
/* CORS con allowlist: se rechazan orígenes que no sean la app o dev local. */
...
app.use((req, res, next) => {
  ...
  /* El navegador del pagador hace POST a /api/webpay/* desde el sitio de
     Transbank: su origin no está en la allowlist, pero no expone datos. */
...
/* ---------- descargas ---------- */
app.get('/api/download/order/:orderId/:itemId', (req, res) => {
  ...
  const zip = buildTemplateBundle(catalogById.get(item.id), {
...
app.get('/api/download/free/:itemId', (req, res) => {
  ...
  const zip = buildTemplateBundle(item, { buyerEmail: '', orderId: 'gratuita' });
...
const officeCatalog = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'office', 'catalog.json'), 'utf8'),
);
const OFFICE_DIR = path.join(__dirname, 'office', 'assets');
...
app.get('/api/office/:id/download', (req, res) => {
  const item = officeCatalog.find((o) => o.id === req.params.id);
  if (!item) return res.status(404).json({ error: 'Plantilla office no encontrada' });

  const file = path.join(OFFICE_DIR, `${item.id}.${item.kind}`);
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="${item.id}.${item.kind}"`,
  );
  res.sendFile(file);
});
...
/* ---------- preview real (HTML inline de templates-src/<id>) ---------- */
app.get('/api/templates/:id/preview-html', (req, res) => {
...
app.post('/api/checkout', optionalUser, genericLimiter, async (req, res) => {
  try {
    ...
    await saveAndSync(orders[orderId]);
  ...
  try {
    ...
    if (order) {
      ...
      if (authorized) {
        ...
        if (order.couponCode) applyCouponUsage(order.couponCode);
      }
      order.commitDetail = {
      ...
      await saveAndSync(order);
      if (authorized) await fulfill(order);
      return res.redirect(302, `${APP_URL}/checkout/result?token=${order.orderId}`);
    }
...
async function main() {
  ...
  app.listen(PORT, () => {
    ...
    console.log(dbEnabled ? 'Persistencia: Supabase.' : 'Persistencia: JSON local.');
  });
}
